...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const db = getDb();
    const expense = await db.prepare('SELECT * FROM expenses WHERE id = ? AND user_id = ?').get(id, user.id);
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const db = getDb();
    const existing = await db.prepare('SELECT * FROM expenses WHERE id = ? AND user_id = ?').get(id, user.id);
