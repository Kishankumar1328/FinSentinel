...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const db = getDb();
    const budget = await db.prepare('SELECT * FROM budgets WHERE id = ? AND user_id = ?').get(id, user.id);
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const db = getDb();
    const existing = await db.prepare('SELECT * FROM budgets WHERE id = ? AND user_id = ?').get(id, user.id);
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await db.prepare('DELETE FROM budgets WHERE id = ? AND user_id = ?').run(id, user.id);
