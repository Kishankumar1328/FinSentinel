  ...
  return (
    <div className="min-h-screen flex overflow-hidden bg-background">
      ...
      <div className="hidden lg:flex flex-col w-5/12 relative overflow-hidden border-r border-white/5">
        ...
        {/* Floating card */}
        <div className="absolute glass rounded-2xl px-5 py-4 border border-indigo-100 shadow-2xl shadow-indigo-200/50 top-[22%] right-[8%]"
          style={{ animation: 'float-orb 7s ease-in-out infinite' }}>
          <p className="text-[9px] text-slate-400 uppercase font-bold mb-1">Monthly savings rate</p>
          <p className="text-2xl font-black text-slate-900">28.4%</p>
          <p className="text-[10px] text-emerald-600 font-bold mt-0.5">↑ vs last month</p>
        </div>

        <div className="absolute glass rounded-xl px-4 py-3 border border-amber-100 shadow-xl shadow-amber-100/30 bottom-[38%] left-[6%]"
          style={{ animation: 'float-orb 9s ease-in-out 3s infinite' }}>
          <div className="flex items-center gap-2">
            <Zap className="h-4 w-4 text-amber-600" />
            <div>
              <p className="text-[10px] text-slate-500 font-bold">Budget alert</p>
              <p className="text-xs font-bold text-amber-700">Dining 92% used</p>
            </div>
          </div>
        </div>
        ...
        <div className="relative z-10 flex flex-col h-full p-12">
          ...
          <div className="space-y-6 my-auto">
            ...
            <p className="text-slate-700 font-semibold leading-relaxed max-w-xs">
            ...
            <div className="grid grid-cols-2 gap-3 pt-3">
              {STATS.map(({ value, label }) => (
                <div key={label} className="glass-sm rounded-xl p-4 border border-indigo-100">
                  <p className="text-2xl font-black text-indigo-600">{value}</p>
                  <p className="text-xs text-slate-600 font-bold mt-0.5">{label}</p>
                </div>
              ))}
            </div>
          </div>

          <p className="text-xs text-slate-400">© 2026 FinSentinel · All rights reserved</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8 relative z-10">
        <div className="w-full max-w-md space-y-7 fade-up">
          ...
          {/* Heading */}
          <div>
            <h1 className="text-4xl font-black text-black tracking-tight leading-tight">Welcome back</h1>
            <p className="text-sm font-semibold text-slate-700 mt-2">Sign in to your financial command centre</p>
          </div>

          {/* Demo banner */}
          <div className="flex items-start gap-3.5 p-4 rounded-2xl glass border border-indigo-200 shadow-sm">
            <div className="w-8 h-8 rounded-xl bg-indigo-500/10 flex items-center justify-center flex-shrink-0">
              <Shield className="h-4 w-4 text-indigo-600" />
            </div>
            <div>
              <p className="text-sm font-bold text-indigo-700">Demo Mode Available</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label htmlFor="email" className="block text-xs font-bold text-slate-950 uppercase tracking-wider mb-2">Email Address</label>
              <input
                id="email" type="email" value={email} onChange={e => setEmail(e.target.value)}
                placeholder="you@example.com" required disabled={loading}
                className="input-glow w-full h-12 px-4 rounded-xl text-sm font-semibold text-slate-950 placeholder-slate-400 disabled:opacity-50"
              />
            </div>

            <div>
              <div className="flex items-center justify-between mb-2">
                <label htmlFor="password" className="text-xs font-bold text-black uppercase tracking-wider">Password</label>
                <a href="#" className="text-xs text-indigo-600 font-bold hover:text-indigo-700 transition-colors">Forgot password?</a>
              </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-bold text-slate-500 uppercase">don't have an account?</span>
            <div className="flex-1 h-px bg-slate-200" />
          </div>

          <Link href="/auth/signup">
            <button className="btn-glass w-full h-11 rounded-xl font-bold text-sm text-slate-950 hover:bg-slate-50 flex items-center justify-center gap-2">
              Create Free Account
            </button>
          </Link>
